...
def get_prompts():
    return RobotSystemPrompts(
        ...
        "\n"
        "BAG RECORDING TOOLS:\n"
        "• rosbag_record(duration, topics) - Record ROS topics to a bag file for later playback\n"
        "• rosbag_play(bag_file, rate=1.0) - Play back a recorded bag file\n"
        "• rosbag_list() - List all recorded bag files\n"
        "• rosbag_info(bag_file) - Get information about a bag file\n"
        "• rosbag_delete(bag_file) - Delete a bag file\n"
        "When asked to record topics, USE rosbag_record() with the specified duration and topic list. "
        "The bag will be saved automatically to the 'bags/' directory and the tool returns the saved path. "
        "Do NOT say you cannot record bags or suggest manual rosbag commands - use rosbag_record() directly.\n"
        "\n"
